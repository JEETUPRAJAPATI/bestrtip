...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($image_id && $section_id) {
        ...
        if ($image) {
            ...
            header("Location: add_package.php?crm=$encryptedId");
            exit;
        } else {
            echo "<script>alert('Image not found!'); window.location.href = 'add_package.php?crm=$encryptedId';</script>";
        }
    } else {
        echo "<script>alert('Invalid request!'); window.location.href = 'add_package.php?crm=$encryptedId';</script>";
    }
}
